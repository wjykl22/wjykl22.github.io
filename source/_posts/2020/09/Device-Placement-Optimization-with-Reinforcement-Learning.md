...
如果直接对$r(\mathcal{P})$并不是很合适，存在如下两个问题：（1）刚开始训练的时候，由于不好的采样，导致$r(\mathcal{P})$会有很多噪音，导致不好的学习信号；（2）快要达到收敛之后，采样变得非常相似，微小的变化导致学习信号的可识别性比较差。所以这里采用$R(\mathcal{P})=\sqrt{r(\mathcal{P})}$代替，鲁棒性更好。
